...
def _build_tree(M):
    ...
    for i in reversed(range(len(M))):
        ...
        # Case 2: only the first child is a parent node
            ...
            sub_newicks[idx] = '(' + sub_newicks[idx].replace(child1, f'{child1},{child2}){parent}')
        ...
        # Case 3: only the second child is a parent node (similar to Case 2)
            ...
            sub_newicks[idx] = '(' + sub_newicks[idx].replace(child2, f'{child2},{child1}){parent}')
            parent_nodes[idx] = parent
...
def _build_tree_with_branch_lengths(M, branch_lengths):
    ...
    for i in range(1, M.shape[0]):
        ...
        current_node.add_child(name=M[i, 1], dist=branch_lengths[f'{M[i, 0]}->{M[i, 1]}'])
        current_node.add_child(name=M[i, 2], dist=branch_lengths[f'{M[i, 0]}->{M[i, 2]}'])
...
def _get_dummy_branch_lengths(M):
    ...
    for i in range(k):
        ...
        for child in children:
            if child <= k:
                branch_lengths[f'{parent}->{child}'] = (k - node_depth.get(parent, i))/k
            else:
...
def newick2v(nw, n_leaves=None, ete3_format=1, reroot=False):
    ...
    try:
        for _ in range(n_leaves-1):
            ...
            nw = nw.replace(f'({left_leaf},{labels[right_leaf]})', str(labels[int(left_leaf_ind)]))
            nw = nw.replace(f'({labels[right_leaf]},{left_leaf})', str(labels[int(left_leaf_ind)]))
...
def change_v(v_old, taxa_dict_old, ete3_format=9):
    ...
    # Reorder M
    M_new, taxa_dict_new = _reorder(M_old, taxa_dict_old)
    ...
    v_new = newick2v(
        tree.write(format=ete3_format),
        ete3_format=ete3_format,
        n_leaves=len(v_old) + 1
    )[1:]

    return v_new, taxa_dict_new


def _reorder(M_old, taxa_dict_old, reorder_internal=True):
...
def _reorder_simple(M_old, taxa_dict_old):
...
def get_equivalent_v(v, rooted):
    """Get all possible re-rootings of v
    ...
    rooted : bool
        True if the tree is rooted otherwise False

    Returns
    -------
    v_store : numpy.ndarray
        List of v with different rerootings
    """
    _, _, tree = label_tree(v, rooted=rooted, ete3_format=9)

    n_nodes = 2 * len(v) + 1

    v_store = np.zeros((n_nodes - 1, len(v)), dtype=np.int16)

    for i, node in enumerate(tree.traverse("preorder")):
        # Avoid error: "cannot set myself as an outgroup"
        if node.is_root():
            continue
        if not node.is_leaf():
            node.name = f'internal{i}'

        tree.set_outgroup(node.name)

        new_v = newick2v(tree.write(format=9))[1:]
